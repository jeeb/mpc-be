...
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "mpc-be_2012.sln %BUILDCFG% Filter %1 - Compilation failed!"
) ELSE (
  CALL :SubMsg "INFO" "mpc-be_2012.sln %BUILDCFG% Filter %1 compiled successfully"
)
EXIT /B
...
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "mpc-be_2012.sln %BUILDCFG% %1 - Compilation failed!"
) ELSE (
  CALL :SubMsg "INFO" "mpc-be_2012.sln %BUILDCFG% %1 compiled successfully"
)
EXIT /B
...
IF %ERRORLEVEL% NEQ 0 (
  ...
  CALL :SubMsg "INFO" "mpciconlib_2012.sln %1 compiled successfully"
)
...
ECHO        Debug only applies to mpc-be_2012.sln.
