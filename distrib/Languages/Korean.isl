...
; 
; 번역은 기본적으로 직역이 원칙이나, 한글로 직역시 어색하거나 매끄럽지 않은 부분은 의역을 하였습니다.
; 제가 번역한 문장들이 원본과 뜻이 다르거나 좀 더 매끈한 표현으로 바꿨으면 하는 부분, 그 밖에 건의 사항이나 질문이 있으시면 제 싸이월드에 방문하셔서 방명록에 글을 남겨주시기 바랍니다.
...
; *** SetupLdr messages
SetupLdrStartupMessage=이 프로그램은 %1를 설치할 것입니다. 계속 하시겠습니까?
...
SetupFileMissing=설치 폴더에 %1 파일이 없습니다. 문제를 해결하시거나 새로운 설치 프로그램을 구해보십시오.
SetupFileCorrupt=설치 파일이 손상되었습니다. 새로운 설치 프로그램을 구해보십시오.
SetupFileCorruptOrWrongVer=설치 파일이 손상되었거나 이 버전의 설치 프로그램과 호환이 되지 않습니다. 문제를 해결하시거나 새로운 설치 프로그램을 구해보십시오.
...
MissingWOW64APIs=실행 중인 Windows 가 설치 프로그램이 64비트 설치를 수행하기 위한 기능을 포함하고 있지 않습니다. 이 문제를 해결하시려면, 서비스 팩 %1 를 설치하십시오.
...
AboutSetupNote=
TranslatorNote=이 번역문은 안웅재가 만들었습니다. 이 번역문에 대한 문의 사항은 Cyworld: http://cyworld.nate.com/nuclear_mine 으로 해 주시기 바랍니다.
...
SelectLanguageLabel=설치하면서 사용할 언어를 선택해 주십시오:
...
SelectDirDesc=어디에 [name] 를 설치하시겠습니까?
SelectDirLabel3=설치 프로그램은 [name] 를 다음 폴더에 설치할 것입니다.
...
SelectComponentsLabel2=설치하고 싶은 구성요소를 선택하시고, 설치하고 싶지 않은 구성요소는 선택을 해제하십시오. 설치를 계속할 준비가 되셨으면 "다음"을 클릭하십시오.
...
NoUninstallWarning=설치 프로그램이 다음 구성 요소가 이미 설치되어 있음을 발견했습니다.%n%n%1%n%n프로그램 제거 시 이 구성요소들은 제거되지 않을 것입니다.%n%n그래도 계속 하시겠습니까?
ComponentSize1=%1 KB
ComponentSize2=%1 MB
ComponentsDiskSpaceMBLabel=선택한 구성요소 설치에 필요한 최소 용량: [mb] MB

; *** "Select Additional Tasks" wizard page
WizardSelectTasks=추가 사항 적용
SelectTasksDesc=어떤 사항을 추가로 적용하시겠습니까?
SelectTasksLabel2=[name] 의 설치하면서 추가로 적용하고자 하는 사항을 선택하시고, "다음"을 클릭하십시오.
...
ReadyLabel1=[name] 를 설치할 준비가 되었습니다.
...
InstallingLabel=설치 프로그램이 [name] 를 설치하는 동안 기다려 주십시오.
...
FinishedRestartLabel=[name] 의 설치를 완료하려면 시스템이 다시 시작되어야 합니다. 지금 시스템을 다시 시작하시겠습니까?
FinishedRestartMessage=[name] 의 설치를 완료하려면 시스템이 다시 시작되어야 합니다.%n%n지금 시스템을 다시 시작하시겠습니까?
...
SelectDiskLabel2=디스크 %1 를 넣고 "확인" 단추를 클릭하십시오.%n%n만약 다음 디스크가 아래에 표시된 경로와 다른 폴더에 있다면 올바른 경로를 입력하시거나 "찾아보기" 단추를 클릭해 올바른 경로를 지정해 주십시오.
PathLabel=경로(&P):
...
ErrorRestartReplace=재시작후 파일교체 실패:
ErrorRenamingTemp=대상 폴더 내의 파일 이름 변경 도중 오류 발생:
ErrorRegisterServer=다음 DLL/OCX 를 등록할 수 없음: %1
