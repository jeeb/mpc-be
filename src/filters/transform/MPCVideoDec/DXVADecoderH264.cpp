...
HRESULT CDXVADecoderH264::DecodeFrame (BYTE* pDataIn, UINT nSize, REFERENCE_TIME rtStart, REFERENCE_TIME rtStop)
{
	...
	if (!m_DXVAPicParams.field_pic_flag && nOutPOC == INT_MIN) {
		TRACE_H264 ("CDXVADecoderH264::DecodeFrame() : Skip Frame, nFramePOC = %11d, nOutPOC = %11d[%11d], [%d - %d]", nFramePOC, nOutPOC, m_nOutPOC, m_DXVAPicParams.field_pic_flag, m_DXVAPicParams.RefPicFlag);
		return S_FALSE;
	}
	...
	bool bAdded = false;
	{
		...
		if (SecondFieldOffset) {
			...
			bAdded = AddToStore (nSurfaceIndex, pSampleToDeliver, m_DXVAPicParams.RefPicFlag, rtStart, rtStop,
								...
								(FF_SLICE_TYPE)nSliceType, nFramePOC, nOutPOC);
		}
	}
