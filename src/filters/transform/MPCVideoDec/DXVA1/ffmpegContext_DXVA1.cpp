...
static void CopyScalingMatrix(DXVA_Qmatrix_H264* pDest, PPS* pps, bool IsATIUVD)
{
...
HRESULT FFH264BuildPicParams(struct AVCodecContext* pAVCtx, DXVA_PicParams_H264* pDXVAPicParams, DXVA_Qmatrix_H264* pDXVAScalingMatrix, bool IsATIUVD)
{
	...
	if (cur_sps && cur_pps) {
		...
		int field_pic_flag										= (h->picture_structure != PICT_FRAME);
		...
		CopyScalingMatrix(pDXVAScalingMatrix, cur_pps, IsATIUVD);
...
BOOL FFH264IsRefFrameInUse(int nFrameNum, struct AVCodecContext* pAVCtx)
{
	...
	for (int i = 0; i < h->short_ref_count; i++) {
		if (nFrameNum == (int)h->short_ref[i]->f.opaque) {
			return TRUE;
		}
	}

	for (int i = 0; i < h->long_ref_count; i++) {
		if (nFrameNum == (int)h->long_ref[i]->f.opaque) {
			return TRUE;
		}
	}
