/*
 * $Id: MpegSplitter.cpp 4404 2012-04-16 10:31:14Z XhmikosR $
 *
 * (C) 2003-2006 Gabest
	...
	, m_AlternativeDuration(false)
{
	...
	if (ERROR_SUCCESS == key.Open(HKEY_CURRENT_USER, _T("Software\\Gabest\\Filters Next\\MPEG Splitter"), KEY_READ)) {
		DWORD dw;
...
#ifdef REGISTER_FILTER
	CRegKey key;
	if (ERROR_SUCCESS == key.Create(HKEY_CURRENT_USER, _T("Software\\Gabest\\Filters Next\\MPEG Splitter"))) {
