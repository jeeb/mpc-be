/*
 * $Id: tta_file.cpp 498 2012-06-05 23:09:29Z Aleksoid $
 *
...
TTA_codec* tta_codec_new(long srate, short num_chan, short bps, short format) {
	...
	ttacodec->enc = ttacodec->tta = (TTA_channel_codec*)tta_alloc(ttacodec->codec_num_chan *
										 ...
										 unsigned long *bytes_to_write)
{
	...
	convert_input_buffer(ttaenc->data + (ttaenc->data_len * ttaenc->num_chan),
		src,
		ttaenc->byte_size,
		consume_data * ttaenc->num_chan);
...
unsigned long tta_codec_decoder_decompress(TTA_codec* ttadec, unsigned char *src,
	unsigned long len, unsigned char *dst, unsigned long dstlen)
{
...
void tta_parser_seek(TTA_parser* ttaparser, uint64 seek_pos_100ns)
{
	...
	if(tta_parser_eof(ttaparser))
		return;
	ttaparser->io->seek(ttaparser->io,
		ttaparser->SeekOffsetTable[ttaparser->FrameIndex], SEEK_SET);
}
