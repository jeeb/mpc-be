...
"%MSBUILD%" mpc-be.sln %MSBUILD_SWITCHES%^
 ...
 /flp1:LogFile=%LOG_DIR%\mpc-be_errors_%BUILDCONFIG%_%1.log;errorsonly;Verbosity=diagnostic^
 /flp2:LogFile=%LOG_DIR%\mpc-be_warnings_%BUILDCONFIG%_%1.log;warningsonly;Verbosity=diagnostic
IF %ERRORLEVEL% NEQ 0 (
  CALL :SubMsg "ERROR" "mpc-be.sln %BUILDCONFIG% %1 - Compilation failed!"
) ELSE (
  CALL :SubMsg "INFO" "mpc-be.sln %BUILDCONFIG% %1 compiled successfully"
)
EXIT /B


:SubResources
TITLE Compiling mpciconlib BE - Release^|%1...
...
FOR %%A IN ("Armenian" "Basque" "Belarusian" "Catalan" "Chinese Simplified" "Chinese Traditional"
 ...
 TITLE Compiling mpcresources BE - %%~A^|%1...
...
ECHO. & ECHO.
ECHO Usage:
ECHO %~nx0 [Clean^|Build^|Rebuild] [x86^|x64^|Both] [Main^|Resource^|MPCBE^|Filters^|All] [Debug^|Release]
ECHO.
